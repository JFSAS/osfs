...
PRIVATE void 	tty_do_vim	(TTY* tty, MESSAGE* msg);
...
PRIVATE void tty_do_read(TTY* tty, MESSAGE* msg)
{
	...
	if (msg->type == VIM) tty_do_vim(tty, msg);
...
PRIVATE void tty_do_vim(TTY* tty, MESSAGE* msg)
{
	int i = 0;
	char buf[500];
	int count = strlen(msg->filenames);
	memcpy(buf, msg->filenames, count); //发现消息再增加叔祖会报错就用了filenames
	buf[count] = '\0';
	while (i < count) {
