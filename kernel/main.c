...
void shabby_shell(const char * tty_name)
{
	...
	while (1) {
        ...
        int command_cnt = 1;
        int argc_cnt = 0;
        char* all_argv[128][128];
        for(i = 0; i < argc; i++){
            if(argv[i][0] != '&') {
                all_argv[command_cnt - 1][argc_cnt++] = argv[i];
            }
            else{
                all_argv[command_cnt - 1][argc_cnt] = 0;
                command_cnt++;
                argc_cnt = 0;
            }
        }
        ...
        for(i = 0; i < command_cnt; i++ ){
			if(strlen(all_argv[i][0]) == 0) continue;
            int fd = open(all_argv[i][0], O_RDWR);

			if(strcmp(all_argv[i][0], "vim") == 12){

				fd = open(all_argv[i][1], O_RDWR);
				int buffer_size = 1024;
				char buf[buffer_size];
				int n = read(fd, buf, buffer_size);

				int r = vim(0, buf, rdbuf, 70);	
				write(fd, rdbuf, r);		
